...
6b. Slope/y-intercept can be shown and typed as **fractions or decimals**. A small toggle button beside the equation switches notation (added to the Tutorial tour), and it is also inferred from what the student types — typing `1/2` switches to and keeps fractions, typing `.5` switches to decimals, a bare integer leaves the mode alone. The preference persists in `slope-invaders:number-format` (fraction default) and is purely presentation: the underlying value stays a number, so scoring/adaptivity/hit detection are untouched. Pure `src/game/logic/rational.ts` (`toRational`/`formatValue`/`parseValue`/`detectFormat`) backs it and `logic/lineMath.equationString` takes an optional `notation`; the steppers still nudge by the same numeric step.
