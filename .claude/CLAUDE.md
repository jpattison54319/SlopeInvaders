...
6b. Slope/y-intercept can be shown and typed as **fractions or decimals**. In the equation, fractions render as a **vertical stack** (numerator over a bar over denominator) via `EquationView`/`StackedFraction`, not inline "1/2". A small toggle button beside the equation switches notation (added to the Tutorial tour, right after Command Center) — it shows a mini stacked ½ in fraction mode and `0.5` in decimal mode. Notation is also inferred from what the student types — `1/2` switches to and keeps fractions, `.5` switches to decimals, a bare integer leaves the mode alone. The preference persists in `slope-invaders:number-format` (fraction default) and is purely presentation: the underlying value stays a number, so scoring/adaptivity/hit detection are untouched. Pure `src/game/logic/rational.ts` (`toRational`/`formatValue`/`parseValue`/`detectFormat`) and `logic/lineMath.ts` (`equationTokens`/`equationString`, optional `notation`) back it; the steppers still nudge by the same numeric step.
...
- `src/game/logic/rational.ts` (pure, tested) does fraction/decimal formatting + parsing for the notation toggle; `src/game/logic/numberFormatStorage.ts` persists the preference (`slope-invaders:number-format`). `logic/lineMath.ts` exposes `equationTokens` (text/fraction tokens) plus the string `equationString`; `components/EquationView.tsx` renders those tokens, drawing fractional coefficients as a **vertical stacked fraction** (`StackedFraction`: numerator over a CSS bar over denominator) while keeping a `visually-hidden` string for screen readers. The notation toggle shows a mini stacked ½ (fraction) or `0.5` (decimal). `EquationControls.tsx` consumes all of it; `Game.tsx`, `ArcadeGame.tsx`, and `VersusMatchScreen.tsx` own the `notation` state and pass it down.
...
- `slope-invaders:number-format` stores the fraction-vs-decimal display/entry preference (`'fraction'` default), shared by Campaign/Arcade/Versus. Presentation only — slope/y-intercept stay numbers, so it never touches scoring, adaptivity, or hit detection.
