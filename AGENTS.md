...
- Campaign opens an atmospheric galaxy where each zone is a planet on a rotating
  ...
  zone/level screens.
- Planet art lives in `src/assets/planets/` (wired via `assetMap`); Tutorial and
  Zones 1-9 each use a unique planet, with worlds becoming more dramatic toward
  the capstone. Each zone unlocks once the previous zone is fully cleared.
...
- Badges (`src/game/campaign/badges.ts`, persisted in `slope-invaders:badges`) reward zone mastery, sharpshooting, and growth behaviors; every badge has a unique emblem (zone planet or `src/assets/achievements/` skill icon), and they are never revoked, never keyed on calculator/tweak use, and are announced in the victory overlay. `markComplete` returns the run's `CompletionRewards` (XP award + new badges + new cosmetics).
...
- Slope/y-intercept show + accept fractions or decimals; equation fractions render as a vertical stack (numerator over a bar over denominator) via `EquationView`/`StackedFraction`. A toggle beside the equation switches notation (in the Tutorial tour, mini stacked ½ / `0.5`); typing also infers it (`1/2` → fractions, `.5` → decimals, bare integer leaves it alone). Persists in `slope-invaders:number-format` (fraction default); presentation-only, so the value stays numeric and scoring/adaptivity/hit detection are untouched. Pure `src/game/logic/rational.ts` + `lineMath.equationTokens` back it.
...
- Struggling-learner support is in-level and non-stigmatizing: miss feedback escalates with consecutive misses on the same target (`escalateMissFeedback`, `src/game/logic/hints.ts`) — sharper nudge → name the lever ("Training Assist") → reveal the needed slope and briefly restore the aiming line ("Slope Scanner", never on no-preview levels). A free **Hint** button and a **Help drawer** (objective recap, per-zone math refresher via `conceptHelp.ts`, controls/tech help) add on-demand help-seeking. None of these affect score or expose the difficulty tier.
- Cooperative class goal + teacher-only leaderboard (cloud-gated, `0004_class_goals.sql`): teachers set one shared goal (stars/levels earned together); students see only the collective class total (no ranking); the dashboard adds a teacher-only sortable leaderboard. Keep the learner view comparison-free.
...
- The learning goal shows in a full-width mission banner (with an "Objective" eyebrow and a brief entrance highlight) above the board (the old per-level teaching "callout" is gone). On a level's first open it is also shown in a one-time **Mission Briefing** modal the player must dismiss with "Begin Mission", so the objective is read rather than skipped. The briefing is suppressed on guided-tour levels such as the Tutorial (their tour already covers the objective). Seen-state persists per level.
- Levels can opt into a one-time guided spotlight tour on first open; the Tutorial uses it. The tour walks every HUD element — including the calculator and the keyboard controls — and each control button prints its keyboard shortcut so players discover the keys without being told.
