...
- Slope/y-intercept show + accept fractions or decimals. A toggle beside the equation switches notation (in the Tutorial tour), and typing infers it (`1/2` → fractions, `.5` → decimals, bare integer leaves it alone). Persists in `slope-invaders:number-format` (fraction default); presentation-only, so the value stays numeric and scoring/adaptivity/hit detection are untouched. Pure `src/game/logic/rational.ts` backs it.
