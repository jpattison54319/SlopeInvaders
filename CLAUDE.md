...
- Slope/y-intercept can be shown and typed as fractions or decimals. A toggle button beside the equation switches notation (in the Tutorial tour), and typing infers it (`1/2` → fractions, `.5` → decimals, bare integer leaves it alone). The preference persists in `slope-invaders:number-format` (fraction default) and is presentation-only — the value stays numeric, so scoring/adaptivity/hit detection are untouched. Backed by pure `src/game/logic/rational.ts`.
