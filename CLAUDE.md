...
- Badges (`src/game/campaign/badges.ts`, persisted in `slope-invaders:badges`) cover zone mastery, sharpshooting, and growth; never revoked, never keyed on calculator/tweak use, announced in the victory overlay. `useCampaignProgress.markComplete` returns the run's `CompletionRewards` (XP award + new badges + new cosmetics).
