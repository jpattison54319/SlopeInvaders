...
- Campaign opens an atmospheric galaxy where zones are planets on a rotating
  ...
  Tutorial and Zones 1-9 use distinct planets that grow more visually dramatic
  toward the capstone.
...
- Badges (`src/game/campaign/badges.ts`, persisted in `slope-invaders:badges`) cover zone mastery, sharpshooting, and growth; every badge has a unique emblem (its unique zone planet or a pixelized skill icon from `src/assets/achievements/`). Badges are never revoked, never keyed on calculator/tweak use, and are announced in the victory overlay. `useCampaignProgress.markComplete` returns the run's `CompletionRewards` (XP award + new badges + new cosmetics).
- Cosmetic rewards (`src/game/campaign/cosmetics.ts`, persisted in `slope-invaders:unlocks`) are unlockable ship hulls, laser styles, and cockpit themes earned by learning behavior — clearing a zone, banking lifetime XP, collecting mastery stars, or earning a badge (every zone clear grants one). They are **purely visual**: equipping never changes hit detection, scoring, hearts, or adaptivity. New unlocks are announced in the victory overlay; the **Hangar** (`src/app/HangarScreen.tsx`, opened from the menu top bar and Pilot Profile) browses and equips them, with locked items showing how to unlock. The equipped selection persists in `slope-invaders:loadout` (`useLoadout`); the active theme recolors the whole app and board. Never gate cosmetics on speed, grinding, or tool use, and keep them non-stigmatizing.
...
- In-level support for strugglers: miss feedback escalates by consecutive misses on the same target (`escalateMissFeedback` in `src/game/logic/hints.ts`) — sharper nudge → name the lever as a "Training Assist" → reveal the needed slope and temporarily restore the aiming line ("Slope Scanner", suppressed on no-preview levels). A free **Hint** button coaches the current aim on demand, and a **Help drawer** (`HelpDrawer.tsx` + per-zone `conceptHelp.ts`) gives the objective, a math refresher, and controls/tech help. All are free, never scored, and never reveal the teacher-only difficulty tier.
- Cooperative class goal + teacher-only leaderboard (cloud-gated, `0004_class_goals.sql`): a teacher sets one shared goal (stars/levels earned together); students see only the collective class total as a progress bar (no ranking); the teacher dashboard adds a teacher-only sortable leaderboard. Pure display math in `src/cloud/classGoal.ts`.
- The learning goal sits in a full-width mission banner (with an "Objective" eyebrow and a brief entrance highlight) above the board (the old per-level teaching "callout" is gone). On a level's first open the objective is also shown in a one-time **Mission Briefing** modal the player dismisses with "Begin Mission" so it is read, not skipped; it is suppressed on guided-tour levels (e.g. the Tutorial) and its seen-state persists per level.
- Levels can opt into a one-time guided spotlight tour on first open; the Tutorial uses it. The tour walks every HUD element including the calculator and keyboard controls, and each gameplay control button prints its keyboard shortcut so the keys are discoverable in-context.
...
- Slope/y-intercept can be shown and typed as fractions or decimals. In the equation, fractions render as a vertical stack (numerator over a bar over denominator) via `EquationView`/`StackedFraction`. A toggle button beside the equation switches notation (in the Tutorial tour) and shows a mini stacked ½ / `0.5`; typing also infers it (`1/2` → fractions, `.5` → decimals, bare integer leaves it alone). The preference persists in `slope-invaders:number-format` (fraction default) and is presentation-only — the value stays numeric, so scoring/adaptivity/hit detection are untouched. Backed by pure `src/game/logic/rational.ts` + `lineMath.equationTokens`.
